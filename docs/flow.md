...
flowchart TD
  ...
  connectionsMenu -->|add| addConn["Prompt name endpoint creds ns db table format"]
  ...
  createMig --> hasFormat{connection or project format set?}
  hasFormat -->|no| pickFormat["Select surql or ts"]
  pickFormat --> saveFormat[Persist migrationFormat on connection]
